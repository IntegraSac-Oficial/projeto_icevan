    ...
    <div className="p-6">
      <div className="max-w-5xl mx-auto">
            ...
            onClick={async () => {
              ...
              try {
                const res = await fetch("/api/admin/images/convert-to-webp", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ folder: "images/hero" }),
                });
                
                const data = await res.json();
                
                if (res.ok && data.ok) {
                  alert(`✅ Conversão concluída!\n\n${data.converted} imagens convertidas\n${data.skipped} já eram WebP\n${data.failed} falharam\n\nRedução total: ${data.totalReduction}\n\n${data.databaseUpdated ? '🗄️ Banco de dados atualizado automaticamente!' : ''}`);
                  await fetchBanners();
                } else {
                  alert(`❌ Erro na conversão: ${data.error || "Erro desconhecido"}`);
                }
              } catch (error) {
                console.error("Erro ao converter banners:", error);
                alert("❌ Erro ao converter banners. Verifique o console.");
              } finally {
                setConverting(false);
              }
            }}
            disabled={converting || loading || banners.length === 0}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
          ...
          {/* Aviso sobre conversão automática */}
          <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
            <p className="text-xs font-semibold text-green-700 mb-1">
              ✨ Conversão Automática para WebP
            </p>
            <p className="text-xs text-green-600">
              Todas as novas imagens enviadas são automaticamente convertidas para WebP (formato otimizado, 30-70% menor). 
              Para converter banners existentes, use o botão &quot;Converter para WebP&quot; acima.
            </p>
          </div>
